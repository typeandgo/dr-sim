...
export const EN = {
  ...
  'policy.statusBlock': "EPs without a rule are blocked",
  'policy.statusPass': "EPs without a rule pass through",
  'policy.faultNote': 'blocked EPs return {fault}',
  'policy.ruleCount': '{count} rules',

  // --- arıza
  'fault.networkError': 'network error',
  'fault.timeout': 'timeout after {seconds} s',
  'fault.preset.http503': '503 Service Unavailable — the server answers with an error',
  'fault.preset.http500': '500 Internal Server Error — the server answers with an error',
  'fault.preset.http429': '429 Too Many Requests — the server answers with an error',
  'fault.preset.http404': '404 Not Found — the server answers, saying the endpoint does not exist',
  'fault.preset.network': 'Network error — the request never lands, there is no status code (same as DevTools “Block request URL”)',
  'fault.preset.timeout': 'Timeout — the request hangs and never gets an answer',
  ...
  'options.faultHint': 'Every blocked request fails this way, and the status line in the panel shows the selected type. Two families, two different app behaviours: with an HTTP fault the server answers, so whatever path the app has for that status code runs — on 5xx many apps escalate to a full-page error. With network error and timeout the request never lands and there is no status code at all, so usually only the failing widget breaks. Try the same EP with both.',
  'options.rules': 'Rules',
...
export const TR: Record<MessageKey, string> = {
  ...
  'policy.statusBlock': "Kural yazılmayan EP'ler bloklanıyor",
  'policy.statusPass': "Kural yazılmayan EP'ler geçiyor",
  'policy.faultNote': "engelli EP'ler {fault} alıyor",
  'policy.ruleCount': '{count} kural',

  'fault.networkError': 'network error',
  'fault.timeout': '{seconds} sn sonra timeout',
  'fault.preset.http503': '503 Service Unavailable — sunucu hata yanıtı döner',
  'fault.preset.http500': '500 Internal Server Error — sunucu hata yanıtı döner',
  'fault.preset.http429': '429 Too Many Requests — sunucu hata yanıtı döner',
  'fault.preset.http404': '404 Not Found — sunucu yanıt verir, “böyle bir endpoint yok” der',
  'fault.preset.network': 'Network error — istek hiç ulaşmaz, durum kodu yoktur (DevTools’taki “Block request URL” ile aynı)',
  'fault.preset.timeout': 'Timeout — istek yanıt alamadan asılı kalır',
  ...
  'options.faultHint': 'Bloklanan her istek bu şekilde başarısız olur; paneldeki durum satırı seçili tipi yazar. İki aile, uygulamada iki farklı davranış: HTTP arızasında sunucu yanıt verir, yani uygulamanın o durum koduna özel yolu çalışır — 5xx’te çoğu uygulama tam sayfa hataya düşer. Network error ve timeout’ta istek hiç ulaşmaz ve durum kodu diye bir şey olmaz, genelde yalnızca ilgili widget kırılır. Aynı EP’yi iki tipte de dene.',
