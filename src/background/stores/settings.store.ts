...
export const normalizeSettings = (raw: unknown): Settings => {
  ...
  // TEK ŞEMA (Revizyon 60): göç zinciri yok. Başka bir sürümden gelen kayıt
  // onarılmaz, TAMAMEN atılır — kullanıcı her şeye son biçimle sıfırdan başlar.
  // Göç kodu tutmanın bedeli, her şema değişikliğinde geriye dönük bir dönüşüm
  // yolunu daha doğru tutmak ve test etmekti; ürün bunu istemiyor.
  if (data.schemaVersion !== SCHEMA_VERSION) return { ...DEFAULT_SETTINGS };
...
export const createSettingsStore = (): SettingsStore => {
  ...
  const load = async (): Promise<Settings> => {
    ...
    try {
      ...
      // profillerini kaybediyor — neden kaybettiğini bilmeli.
      const outdated = !!raw && typeof raw === 'object'
        && (raw as Record<string, unknown>).schemaVersion !== SCHEMA_VERSION;

      if (outdated) notice = 'settings-reset';
      else if (raw && typeof raw !== 'object') notice = 'settings-read';
    } catch {
